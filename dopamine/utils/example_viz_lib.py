...
class MyDQNAgent(dqn_agent.DQNAgent):
    ...
    def __init__(self, sess, num_actions, summary_writer=None):
        super(MyDQNAgent, self).__init__(sess, num_actions, summary_writer=summary_writer)
    ...
    def _select_action(self):
        ...
        q_vals = self._sess.run(self._net_outputs.q_values, {self.state_ph: self.state})[0]
    ...
    def reload_checkpoint(self, checkpoint_path, use_legacy_checkpoint=False):
        if use_legacy_checkpoint:
            ...
            ckpt_vars = ["{}:0".format(name) for name, _ in tf.train.list_variables(checkpoint_path)]
