        ...
        for agent in ["dqn", "c51", "rainbow", "iqn"]:
            ...
            if not tf.io.gfile.exists(game_data_file):
                if verbose:
                    ...
                    print("Unable to load data for agent {} on game {}".format(agent, game))
            ...
            with tf.io.gfile.GFile(game_data_file, "rb") as f:
                ...
                for field_name in single_agent_data.keys():
                    try:
                        single_agent_data[field_name] = single_agent_data[field_name].astype(np.float64)
                    except ValueError:
                        # This will catch any non-numerics that cannot be cast to float64.
                        continue
                if game in experimental_data:
                    experimental_data[game] = experimental_data[game].merge(single_agent_data, how="outer")
                else:
...
def read_experiment(
    ...
    data_frame = pd.DataFrame(index=np.arange(0, expected_num_rows), columns=column_names)
    row_index = 0
    ...
    for parameter_tuple in itertools.product(*ordered_values):
        if job_descriptor is not None:
            ...
            name = "-".join([keys[i] + "_" + str(parameter_tuple[i]) for i in range(len(keys))])

        experiment_path = "{}/{}/logs".format(log_path, name)

        raw_data, last_iteration = load_statistics(experiment_path, iteration_number=iteration_number, verbose=verbose)
        ...
        for iteration in range(last_iteration + 1):
            ...
            # requested values.
            row_data = list(parameter_tuple) + [iteration] + [summary[key][iteration] for key in summary_keys]
