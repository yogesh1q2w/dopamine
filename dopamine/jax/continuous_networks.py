...
class _Tanh(tfb.Tanh):
    ...
    def _inverse(self, y):
        ...
        y = jnp.where(jnp.less_equal(jnp.abs(y), 1.0), jnp.clip(y, -0.99999997, 0.99999997), y)
...
class ActorNetwork(nn.Module):
    ...
    kernel_initializer: jax.nn.initializers.Initializer = jax.nn.initializers.glorot_uniform()
    ...
    def __call__(
        ...
        for _ in range(self.num_layers):
            x = nn.Dense(features=self.hidden_units, kernel_init=self.kernel_initializer)(x)
            x = self.activation(x)
        ...
        loc_and_scale_diag = nn.Dense(features=action_dim * 2, kernel_init=self.kernel_initializer)(x)
...
class ActorCriticNetwork(nn.Module):
    ...
    def __call__(self, state: jnp.ndarray, key: jnp.ndarray, mean_action: bool = True) -> ActorCriticOutput:
    ...
    def critic(self, state: jnp.ndarray, action: jnp.ndarray) -> CriticOutput:
        ...
        """
        return CriticOutput(self._critic1(state, action), self._critic2(state, action))
...
class PPOActorNetwork(nn.Module):
    ...
    def __call__(
        ...
        scale_diag = nn.Dense(features=action_dim, kernel_init=nn.initializers.zeros)(jnp.ones_like(x))
        scale_diag = jnp.exp(scale_diag)
...
class PPOActorCriticNetwork(nn.Module):
    ...
    def setup(self):
        ...
        self._critic = PPOCriticNetwork(self.num_layers, self.hidden_units, self.activation)

    def __call__(self, state: jnp.ndarray, key: jnp.ndarray) -> PPOActorCriticOutput:
        """Calls the actor/critic networks.
