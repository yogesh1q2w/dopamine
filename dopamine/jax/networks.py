...
class Stack(nn.Module):
    ...
    def __call__(self, x):
        ...
        for _ in range(self.num_blocks):
            ...
            conv_out = nn.Conv(features=self.num_ch, kernel_size=(3, 3), strides=1, padding="SAME")(conv_out)
            conv_out = nn.relu(conv_out)
            conv_out = nn.Conv(features=self.num_ch, kernel_size=(3, 3), strides=1, padding="SAME")(conv_out)
...
class ImpalaEncoder(nn.Module):
    ...
    def setup(self):
        ...
        self._stacks = [
            Stack(num_ch=stack_size * self.nn_scale, num_blocks=self.num_blocks) for stack_size in self.stack_sizes
        ]
...
class ClassicControlDQNNetwork(nn.Module):
    ...
    def setup(self):
        ...
        initializer = nn.initializers.xavier_uniform()
        self.layers = [nn.Dense(features=self.hidden_units, kernel_init=initializer) for _ in range(self.num_layers)]
...
class FourierBasis(object):
    ...
    def __init__(
        ...
        if max_vals is not None:
            ...
            self.denominator = [max_vals[i] - min_vals[i] for i in range(len(min_vals))]
...
class JaxFourierDQNNetwork(nn.Module):
    ...
    def __call__(self, x):
        ...
        q_values = nn.Dense(features=self.num_actions, kernel_init=initializer, use_bias=False)(x)
        return atari_lib.DQNNetworkType(q_values)
...
class RainbowNetwork(nn.Module):
    ...
    def __call__(self, x, support):
        ...
        x = nn.Dense(features=512, kernel_init=initializer)(x)
        x = nn.relu(x)
        x = nn.Dense(features=self.num_actions * self.num_atoms, kernel_init=initializer)(x)
...
class ClassicControlRainbowNetwork(nn.Module):
    ...
    def setup(self):
        ...
        initializer = nn.initializers.xavier_uniform()
        self.layers = [nn.Dense(features=self.hidden_units, kernel_init=initializer) for _ in range(self.num_layers)]
        self.final_layer = nn.Dense(features=self.num_actions * self.num_atoms, kernel_init=initializer)
...
class PPODiscreteActorCriticNetwork(nn.Module):
    ...
    def __call__(self, state: jnp.ndarray, key: jnp.ndarray) -> continuous_networks.ActorCriticOutput:
    ...
    def critic(self, state: jnp.ndarray) -> continuous_networks.PPOCriticOutput:
        return continuous_networks.PPOCriticOutput(self._critic(self._shared_network(state)))
