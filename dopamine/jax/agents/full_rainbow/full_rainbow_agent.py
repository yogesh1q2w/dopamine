...
@gin.configurable
def zero_epsilon(unused_decay_period, unused_step, unused_warmup_steps, unused_epsilon):
    return 0.0
...
def select_action(
    ...
    best_actions = jnp.argmax(network_def.apply(params, state, key=rng2, eval_mode=eval_mode, support=support).q_values)
    return rng, jnp.where(p <= epsilon, jax.random.randint(rng3, (), 0, num_actions), best_actions)
...
@functools.partial(jax.vmap, in_axes=(None, None, 0, 0, 0, None, None, None, None, None))
def target_output(
    model,
    target_network,
    next_states,
    ...
    if distributional:
        ...
        target = rainbow_agent.project_distribution(target_support, next_probabilities, support)
    else:
...
class JaxFullRainbowAgent(dqn_agent.JaxDQNAgent):
    ...
    def _build_networks_and_optimizer(self):
        ...
        self.online_params = self.network_def.init(rng, x=state, support=self._support)
    ...
    def _build_replay_buffer(self):
        ...
        sampling_distribution = samplers.PrioritizedSamplingDistribution(seed=self._seed)
    ...
    def _training_step_update(self):
        ...
        if (
            ...
            with self.summary_writer.as_default():
                tf.summary.scalar("CrossEntropyLoss", mean_loss, step=self.training_steps)
    ...
    def _store_transition(self, last_observation, action, reward, is_terminal, *args, priority=None, episode_end=False):
        ...
        if is_prioritized and priority is None:
            if self._replay_scheme == "uniform":
                ...
                priority = self._replay._sampling_distribution._sum_tree.max_recorded_priority
