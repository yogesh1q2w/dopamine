...
def train(
    ...
    def loss_fn(params, rng_input, target_quantile_vals):
        def online(state, key):
            return network_def.apply(params, state, num_quantiles=num_tau_samples, rng=key)
        ...
        huber_loss_case_two = (
            (jnp.abs(bellman_errors) > kappa).astype(jnp.float32) * kappa * (jnp.abs(bellman_errors) - 0.5 * kappa)
        )
        ...
        quantile_huber_loss = (
            jnp.abs(quantiles - jax.lax.stop_gradient((bellman_errors < 0).astype(jnp.float32))) * huber_loss
        ) / kappa
...
def select_action(
    ...
    return rng, jnp.where(
        ...
        jnp.argmax(
            jnp.mean(
                network_def.apply(params, state, num_quantiles=num_quantile_samples, rng=rng2).quantile_values,
                axis=0,
            ),
            axis=0,
        ),
...
class JaxImplicitQuantileAgent(dqn_agent.JaxDQNAgent):
    ...
    def __init__(
        ...
        super(JaxImplicitQuantileAgent, self).__init__(
            ...
            network=functools.partial(network, quantile_embedding_dim=quantile_embedding_dim),
            gamma=gamma,
    ...
    def _build_networks_and_optimizer(self):
        ...
        self.online_params = self.network_def.init(rng, x=self.state, num_quantiles=self.num_tau_samples, rng=self._rng)
