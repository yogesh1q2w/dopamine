...
def train(
    ...
    """
    q_values = jax.vmap(lambda state: network_def.apply(network_params, state, method=network_def.critic).q_value)(
        states
    )
    ...
    next_q_value = network_def.apply(network_params, next_states[-1], rng).critic.q_value
    ...
    sampled_actions = jax.vmap(
        lambda state, key: network_def.apply(network_params, state, key=key, method=network_def.actor).sampled_action
    )(states, batch_keys)
...
def calculate_advantages_and_returns(
    ...
    for t in reversed(range(nsteps)):
        delta = (
            rewards[t]
            + gamma * (q_values[t + 1] if t < nsteps - 1 else next_q_value) * (1.0 - terminals[t])
            - q_values[t]
        )
        advantages[t] = delta + lambda_ * gamma * (advantages[t + 1] if t < nsteps - 1 else 0.0) * (1.0 - terminals[t])
...
def create_minibatches_and_shuffle(
    ...
    assert states.shape[0] % batch_size == 0, "Batch size must divide number of states."
    ...
    advantages = advantages.reshape((num_batches, batch_size) + advantages.shape[1:])
    log_probability = log_probability.reshape((num_batches, batch_size) + log_probability.shape[1:])
...
def train_minibatch(
    ...
    def loss_fn(
        ...
        if clip_critic_loss:
            ...
            critic_loss = 0.5 * jnp.mean(
                jnp.maximum(
                    ...
                    losses.mse_loss(
                        jnp.clip(q_values, old_q_values - epsilon, old_q_values + epsilon),
                        returns,
                    ),
                )
            )
        ...
        loss = actor_loss + vf_coefficient * critic_loss - entropy_coefficient * entropy_loss
    ...
    advantages = jnp.array((advantages - advantages.mean()) / (advantages.std() + 1e-8))
    ...
    updates, optimizer_state = optim.update(grad, optimizer_state, params=network_params)
...
def select_action(network_def, params, state, rng):
    ...
    action = network_def.apply(params, state, rng2, method=network_def.actor).sampled_action
