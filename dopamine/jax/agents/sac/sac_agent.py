...
try:
    ...
    logging.info(("Unable to modify visible devices. " "If you don't have a GPU, this is expected."))
...
def train(
    ...
    def loss_fn(
        ...
        q_value_1, q_value_2 = network_def.apply(params, state, action, method=network_def.critic)
        ...
        target = reward_scale_factor * reward + cumulative_gamma * (target_q_value - alpha_value * log_prob) * (
            1.0 - terminal
        )
        ...
        mean_action, sampled_action, action_log_prob = network_def.apply(params, state, rng2, method=network_def.actor)
    ...
    updates, optimizer_state = optim.update(network_gradient, optimizer_state, params=network_params)
    network_params = optax.apply_updates(network_params, updates)
    alpha_updates, alpha_optimizer_state = alpha_optim.update(alpha_gradient, alpha_optimizer_state, params=log_alpha)
...
def select_action(network_def, params, state, rng, eval_mode=False):
    ...
    greedy_a, sampled_a, _ = network_def.apply(params, state, rng2, method=network_def.actor)
...
class SACAgent(dqn_agent.JaxDQNAgent):
    ...
    def __init__(
        ...
        logging.info("\t target_smoothing_coefficient: %f", target_smoothing_coefficient)
        ...
        self.network_def = network(action_shape, num_layers, hidden_units, action_limits)
        self.gamma = gamma
    ...
    def _build_replay_buffer(self):
        ...
        sampling_distribution = samplers.UniformSamplingDistribution(seed=self._seed)
    ...
    def _maybe_sync_weights(self):
        ...
        if self.target_update_type == "hard" and self.training_steps % self.target_update_period != 0:
            return

        def _sync_weights(target_p, online_p):
            return self.target_smoothing_coefficient * online_p + (1 - self.target_smoothing_coefficient) * target_p

        self.target_params = jax.tree.map(_sync_weights, self.target_params, self.network_params)
                ...
                if (
                    ...
                    for k in train_returns:
                        if k.startswith("Losses") or k.startswith("Values"):
                            self.summary_writer.scalar(k, train_returns[k], self.training_steps)
