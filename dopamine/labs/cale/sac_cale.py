...
def select_action_eps_greedy(
    ...
    return rng, jnp.where(
        p <= epsilon,
        jax.random.uniform(rng2, action_shape, action_dtype, action_limits[0], action_limits[1]),
        sac_agent.select_action(network_def, params, state, rng3, eval_mode=eval_mode)[1],
    )
...
class SACCALEAgent(sac_agent.SACAgent):
    ...
    def _maybe_log_action_distribution(self):
        if self.action_distributions is not None:
            action_int = utils.get_action_number(self.action[0], self.action[1], self.action[2])
