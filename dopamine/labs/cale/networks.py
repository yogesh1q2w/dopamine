...
class NatureDQNEncoder(nn.Module):
    ...
    def __call__(self, x):
        ...
        critic_z = nn.Dense(features=self.projection_dimension, kernel_init=initializer)(x)
        ...
        actor_z = nn.Dense(features=self.projection_dimension, kernel_init=initializer)(jax.lax.stop_gradient(x))
...
class SACImpalaEncoder(nn.Module):
    ...
    def setup(self):
        ...
        self._stacks = [
            discrete_networks.Stack(num_ch=stack_size * self.nn_scale, num_blocks=self.num_blocks)
...
class SACCALEConvNetwork(nn.Module):
    ...
    def actor(self, state: jnp.ndarray, key: jnp.ndarray) -> continuous_networks.ActorOutput:
    ...
    def critic(self, state: jnp.ndarray, action: jnp.ndarray) -> continuous_networks.CriticOutput:
...
class PPOCALEConvNetwork(nn.Module):
    ...
    def __call__(self, state: jnp.ndarray, key: jnp.ndarray) -> continuous_networks.PPOActorCriticOutput:
