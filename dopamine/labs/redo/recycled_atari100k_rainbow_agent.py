...
@gin.configurable
class RecycledAtari100kRainbowAgent(atari_100k_rainbow_agent.Atari100kRainbowAgent):
    ...
    def __init__(
        ...
        super().__init__(num_actions, network=network, summary_writer=summary_writer)
        ...
        if weight_decay > 0:
            ...
            self.optimizer = optax.chain(optax.add_decayed_weights(weight_decay), self.optimizer)
        ...
        if reset_mode is not None:
            if reset_mode == "neurons":
                self.weight_recycler = weight_recyclers.NeuronRecycler(network.layer_names, network=network_name)
    ...
    def _log_stats(self, log_dict, step):
        ...
        self.collector_dispatcher.write(stats, collector_allowlist=self._collector_allowlist)
        ...
        if self._replay.add_count > self.min_replay_history:
            if self.training_steps % self.update_period == 0:
                for step in range(self._num_updates_per_train_step):
                    self.gradient_step = (
                        (self.training_steps // self.update_period) * self._num_updates_per_train_step
                    ) + step
    ...
    def _training_step_update(self):
        ...
        (self.optimizer_state, self.online_params, loss, mean_loss, self._rng) = full_rainbow_agent.train(
            self.network_def,
            self.online_params,
        ...
        is_logging = update_step > 0 and update_step % self.summary_writing_frequency == 0
        if is_logging:
    ...
    def _weight_recycle(self, update_step, online_params):
        ...
        is_intermediated = self.weight_recycler.is_intermediated_required(update_step)
        # get intermediate activation per layer to calculate neuron score
        intermediates = self.get_intermediates(online_params) if is_intermediated else None
        ...
        if self.is_debugging:
            log_dict_intersected = self.weight_recycler.intersected_dead_neurons_with_last_reset(
    ...
    def _sample_batch_for_statistics(self):
        samples = self._replay.sample_transition_batch(batch_size=self.batch_size_statistics)
