...
flags.DEFINE_bool("is_classic", False, "Whether the env is a classic control game.")
flags.DEFINE_float(
    "average_last_fraction",
    0.1,
    "Tells what fraction latest evaluation scores are averaged. This is used" " to reduce variance.",
)
...
def create_sequential_critic_network(
    ...
    obs_network_layers = (
        create_fc_layers(obs_fc_layer_units, width=width, weight_decay=weight_decay) if obs_fc_layer_units else None
    )

    # Create an action network layers.
    action_network_layers = (
        create_fc_layers(action_fc_layer_units, width=width, weight_decay=weight_decay)
    ...
    joint_network_layers = (
        create_fc_layers(joint_fc_layer_units, width=width, weight_decay=weight_decay) if joint_fc_layer_units else None
    )
    ...
    module_list = [
        create_identity_layer() if layers is None else sequential.Sequential(layers) for layers in layer_list
    ]
...
class _TanhNormalProjectionNetworkWrapper(sparse_tanh_normal_projection_network.SparseTanhNormalProjectionNetwork):
    ...
    def __init__(self, sample_spec, predefined_outer_rank=1, weight_decay=0.0):
        super(_TanhNormalProjectionNetworkWrapper, self).__init__(sample_spec=sample_spec, weight_decay=weight_decay)
...
def create_sequential_actor_network(
    ...
    last_layer = nest_map.NestMap(tf.nest.map_structure(tanh_normal_projection_network_fn, action_tensor_spec))

    return sequential.Sequential(dense_layers + [tf.keras.layers.Lambda(tile_as_nest)] + [last_layer])
...
class RecycledSacAgent(sac_agent.SacAgent):
    ...
    def _train(self, experience, weights):
        ...
        trainable_critic_variables = list(
            object_identity.ObjectIdentitySet(
                self._critic_network_1.trainable_variables + self._critic_network_2.trainable_variables
            )
        )
        ...
        with tf.GradientTape(watch_accessed_variables=False, persistent=True) as tape:
            ...
            self.critic1_act_grads = (
                tape.gradient(critic_loss, self.critic1_act) if self.neuron_score_algo == "activation_grad" else None
            )
            self.critic2_act_grads = (
                tape.gradient(critic_loss, self.critic2_act) if self.neuron_score_algo == "activation_grad" else None
            )
        ...
        with tf.GradientTape(watch_accessed_variables=False, persistent=True) as tape:
            ...
            actor_loss = self._actor_loss_weight * self.actor_loss(time_steps, weights=weights, training=True)

            self.actor_act = get_intermedieates(self._actor_network)
            self.actor_act_grads = (
                tape.gradient(actor_loss, self.actor_act) if self.neuron_score_algo == "activation_grad" else None
            )
        ...
        log_w_mean_c1 = functools.partial(self.log_weights_mean, "critic_1", self._critic_network_1)
        tf.cond(is_logging, log_w_mean_c1, lambda: None)
        log_w_mean_c2 = functools.partial(self.log_weights_mean, "critic_2", self._critic_network_2)
        tf.cond(is_logging, log_w_mean_c2, lambda: None)
        log_w_mean_actor = functools.partial(self.log_weights_mean, "actor", self._actor_network)
        ...
        with tf.GradientTape(watch_accessed_variables=False) as tape:
            ...
            alpha_loss = self._alpha_loss_weight * self.alpha_loss(time_steps, weights=weights, training=True)
        ...
        extra = sac_agent.SacLossInfo(critic_loss=critic_loss, actor_loss=actor_loss, alpha_loss=alpha_loss)
    ...
    def log_deadneurons_models(self):
        ...
        self.log_dead_neurons_count(self._actor_network, "actor", self.actor_act, self.actor_act_grads)

    def calculate_neuron_score_all_layers(self, model, intermediate_act, intermediate_act_grad):
        all_layers_score = {}
        if self.neuron_score_algo == "activation":
            for act_key, act_value in intermediate_act.items():
                ...
                neurons_score = tf.reduce_mean(tf.math.abs(act_value[: self.log_batch_size, :]), axis=0)
            ...
            for act, act_grad in zip(intermediate_act.items(), intermediate_act_grad.items()):
                act_k, act_v = act
            ...
            raise ValueError("neuron_score_algo:%s  is not valid." % self.neuron_score_algo)
        return all_layers_score

    def log_dead_neurons_count(self, model, network_name, intermediate_act, intermediate_act_grad):
        ...
        all_layers_score = self.calculate_neuron_score_all_layers(model, intermediate_act, intermediate_act_grad)
        ...
        for layer_count, (layer_name, layer_score) in enumerate(all_layers_score.items()):
            ...
            with tf.name_scope("dead_neurons/" + network_name + "/"):
                tf.compat.v2.summary.scalar("layer_" + str(layer_count + 1) + "_" + layer_name, num_dead_neurons)
    ...
    def reset_weights(self, optimizer, model, target_model=None):
        ...
        model_layers = get_all_layers(model)
        target_model_layers = model_layers if target_model is None else get_all_layers(target_model)
        ...
        for layer, target_layer in zip(model_layers, target_model_layers):
            ...
            for var, target_var in zip(layer.weights, target_layer.weights):
                ...
                k = tf.dtypes.cast(
                    tf.math.maximum(
                        tf.math.round(tf.dtypes.cast(tf.size(abs_weights), tf.float32) * self.reset_frac),
                        1,
                    ),
                    tf.int32,
                )
                ...
                new_weights, mask = self.create_new_weights(k, var, abs_weights, w_initializer, b_initializer)
    ...
    def create_new_weights(self, k, weights, abs_weights, w_initializer, b_initializer):
    ...
    def get_mask_dead_neurons_weights(self, act, act_grad, model):
        model_layers = get_all_layers(model, filter_fn=is_dense_layer)
        all_layers_score = self.calculate_neuron_score_all_layers(model, act, act_grad)
        incoming_masks = {}
        outgoing_masks = {}
        dead_neuron_masks = {}
        for (
            ...
            incoming_mask, outgoing_mask = self.create_mask_helper(mask, incoming_var.shape[0], outgoing_var.shape[1])
    ...
    def reset_dead_neurons(self, optimizer, act, act_grad, model, target_model=None):
        ...
        model_layers = get_all_layers(model, filter_fn=is_dense_layer)
        target_model_layers = (
            model_layers if target_model is None else get_all_layers(target_model, filter_fn=is_dense_layer)
        )
        dead_neuron_masks, incoming_masks, outgoing_masks = self.get_mask_dead_neurons_weights(act, act_grad, model)
        ...
        for layer, target_layer in zip(model_layers[:-1], target_model_layers[:-1]):
            ...
            w_initializer = type(layer.kernel_initializer)(seed=self.train_step_counter)
            ...
            new_in_weights = tf.where(incoming_mask == 1, new_in_weights, incoming_var)

            incoming_var.assign(new_in_weights)
            if self.reset_target_models and target_model is not None:
                new_target_in_weights = tf.where(incoming_mask == 1, new_in_weights, incoming_target_var)
            ...
            new_bias = tf.where(tf.math.equal(dead_neuron_masks[layer.name], 1), new_bias, layer.bias)
        ...
        for layer, next_layer, target_next_layer in zip(model_layers[:-1], model_layers[1:], target_model_layers[1:]):
            outgoing_mask = outgoing_masks[next_layer.name]
            outgoing_var = next_layer.kernel
            outgoing_target_var = target_next_layer.kernel
            if self.init_method == "random":
                w_initializer = type(next_layer.kernel_initializer)(seed=self.train_step_counter)
            ...
            new_out_weights = tf.where(outgoing_mask == 1, new_out_weights, outgoing_var)
            outgoing_var.assign(new_out_weights)
            if self.reset_target_models and target_model is not None:
                new_target_out_weights = tf.where(outgoing_mask == 1, new_out_weights, outgoing_target_var)
    ...
    def _rescale_weights(self, neuron_mask, var, new_weights, axis, scaler):
        ...
        non_recycled_norm = tf.reduce_sum((tf.norm(var, axis=axis) * tf.cast(1 - neuron_mask, tf.float32))) / tf.cast(
            non_dead_count, tf.float32
        )
...
def train_eval(
    ...
    xm_objective_value_train_reward = work_unit.get_measurement_series(label="train_reward")
    ...
    logging.info("SAC params: actor_critic_widths_str: %s", actor_critic_widths_str)
    ...
    reverb_checkpoint_dir = os.path.join(root_dir, learner.TRAIN_DIR, learner.REPLAY_BUFFER_CHECKPOINT_DIR)
    ...
    def experience_dataset_fn():
        return reverb_replay.as_dataset(sample_batch_size=batch_size, num_steps=2).prefetch(50)
    ...
    eval_greedy_policy = py_tf_eager_policy.PyTFEagerPolicy(tf_greedy_policy, use_tf_function=True)
    ...
    logging.info("Step %d, Average Return: %f", env_step_metric.result(), avg_return)
