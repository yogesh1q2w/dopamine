...
def get_gradients(online_params, apply_fn, target_params, batch, cumulative_gamma):
    ...
    target = jax.lax.stop_gradient(batch["reward"] + cumulative_gamma * replay_next_qt_max * (1.0 - batch["terminal"]))
...
class RecycledDQNAgent(dqn_agent.JaxDQNAgent):
    ...
    def _training_step_update(self):
        ...
        batch = {
            k: self.preprocess_fn(v) if k in ["state", "next_state"] else v for k, v in self.replay_elements.items()
        }
