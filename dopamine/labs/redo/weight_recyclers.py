...
class LayerReset(BaseRecycler):
    ...
    def update_weights(self, intermediates, params, key, opt_state):
        ...
        mask_dict = {k: jnp.zeros_like(p) if p.ndim != 1 else None for k, p in param_dict.items()}
        ...
        masks = flax.core.freeze(flax.traverse_util.unflatten_dict(mask_dict, sep="/"))
        random_keys = flax.core.freeze(flax.traverse_util.unflatten_dict(random_keys_dict, sep="/"))
        # reset weights
        weight_random_reset_fn = jax.jit(functools.partial(jax.tree.map, weight_reinit_random))
