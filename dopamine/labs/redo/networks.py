...
class ScalableNatureDQNNetwork(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.relu(x)
        x = self._record_activations(x, layer)
        layer = nn.Dense(features=self.num_actions, kernel_init=initializer, name="final_layer")
        q_values = layer(x)
...
class Stack(nn.Module):
    ...
    def __call__(self, x):
        ...
        for _ in range(self.num_blocks):
            ...
            layer = nn.Conv(features=self.num_ch, kernel_size=(3, 3), strides=1, padding="SAME")
...
class FullRainbowNetwork(nn.Module):
    ...
    def __call__(self, x, support, eval_mode=False, key=None):
        ...
        if self.dueling:
            ...
            layer = nn.Dense(features=self.num_atoms, kernel_init=nn.initializers.xavier_uniform())
            value = layer(x)
