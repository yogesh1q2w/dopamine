...
SPROutputType = collections.namedtuple("RL_network", ["q_values", "logits", "probabilities", "latent"])
...
class SPRNetwork(nn.Module):
    ...
    def __call__(
        ...
        x = self.apply_projection(latent.reshape(-1), key, eval_mode)  # Single hidden layer of size 512
        x = nn.relu(x)
