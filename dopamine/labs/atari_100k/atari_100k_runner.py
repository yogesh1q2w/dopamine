...
class DataEfficientAtariRunner(run_experiment.Runner):
    ...
    def __init__(
        ...
        if game_name is not None:
            ...
            if log_normalized_scores:
                raise ValueError("Game name must not be None if logging normalized scores.")
        ...
        while True:
            ...
            while live_env_index < len(live_envs):
                ...
                if envs[env_id].game_over or envs[env_id].cum_length == self._max_steps_per_episode:
                    ...
                    log_str = "Steps executed: {} Num episodes: {} Episode length: {}" " Return: {} ".format(
                        total_steps,
                        len(cum_rewards),
                        cum_lengths[-1],
                        cum_rewards[-1],
    ...
    def _run_train_phase(self, statistics):
        ...
        if self.log_normalized_scores:
            normalized_return = normalization_utils.normalize_score(average_return, self.game_name)
    ...
    def _run_one_iteration(self, iteration):
        ...
        num_episodes_eval, average_reward_eval, norm_score_eval = self._run_eval_phase(statistics)
    ...
    def _maybe_save_single_summary(self, iteration, ep_return, length, save_if_eval=False):
        ...
        if not self._agent.eval_mode or save_if_eval:
            with self._summary_writer.as_default():
                ...
                if self.log_normalized_scores:
                    normalized_score = normalization_utils.normalize_score(ep_return, self.game_name)
                    tf.summary.scalar(
