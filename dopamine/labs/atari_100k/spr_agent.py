...
def train(
    ...
    def q_target(state, key):
        return network_def.apply(target_params, state, key=key, support=support, mutable=["batch_stats"])

    def encode_project(state, key):
        latent, _ = network_def.apply(target_params, state, method=network_def.encode, mutable=["batch_stats"])
    ...
    def loss_fn(params, target, spr_targets, loss_multipliers):
        ...
        if distributional:
            (logits, spr_predictions) = get_logits(q_online, current_state, actions[:, :-1], use_spr, rng)
            ...
            q_values, spr_predictions = get_q_values(q_online, current_state, actions[:, :-1], use_spr, rng)
    ...
    if use_spr:
        ...
        spr_targets = spr_targets.reshape(*future_states.shape[:2], *spr_targets.shape[1:]).transpose(1, 0, 2)
    else:
        spr_targets = None

    # Get the unweighted loss without taking its mean for updating priorities.
    (mean_loss, (_, dqn_loss, spr_loss)), grad = grad_fn(online_params, target, spr_targets, loss_weights)
...
class SPRAgent(atari_100k_rainbow_agent.Atari100kRainbowAgent):
    ...
    def _training_step_update(self):
        ...
        next_states = self.train_preprocess_fn(self.replay_elements["next_state"][:, 0], rng=rng2)
        ...
        if self._replay_scheme == "prioritized":
            ...
            self._replay.set_priority(self.replay_elements["indices"], jnp.sqrt(dqn_loss + 1e-10))
