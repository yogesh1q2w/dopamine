...
class DeterministicSumTree(sum_tree.SumTree):
    ...
    def sample(self, rng, query_value=None):
        ...
        nodes = jax.device_put(jnp.asarray(self.nodes), jax.local_devices(backend="cpu")[0])
    ...
    def stratified_sample(self, batch_size, rng):
        ...
        rng = jax.device_put(rng, jax.local_devices(backend="cpu")[0])
        nodes = jax.device_put(jnp.asarray(self.nodes), jax.local_devices(backend="cpu")[0])
        indices = parallel_stratified_sample(rng, nodes, np.arange(batch_size), batch_size, self.depth)
