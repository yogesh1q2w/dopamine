...
def create_deepmind_control_environment(
    ...
    """
    env = suite.load(domain_name, task_name, environment_kwargs={"flat_observation": True})
...
class DeepmindControlPreprocessing(gym.Env):
    ...
    def action_space(self) -> spaces.Box:
        ...
        return spaces.Box(low=action_spec.minimum, high=action_spec.maximum, dtype=np.float32)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, Mapping[Any, Any]]:
