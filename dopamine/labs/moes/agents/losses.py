...
def naive_entropy(
    ...
    flattened_bin_counts = jnp.bincount(top_experts.flatten(), length=loss_parameters.num_experts)
...
def importance_loss(
    ...
    return types.MoELossReturn(value=weighted_importance_loss, statistics=statistics)
...
def load_loss(loss_parameters: types.MoELossParameters) -> types.MoELossReturn:
    ...
    def calculate_load_loss(logits, logits_noisy, noise_std, num_selected_experts):
        ...
        threshold_per_item_index = jax.lax.top_k(logits_noisy, loss_parameters.num_selected_experts)[-1][..., -1]
        threshold_per_item = jnp.sum(
            jax.nn.one_hot(threshold_per_item_index, loss_parameters.num_experts) * logits_noisy,
            axis=-1,
        )
    ...
    logits_noise = noise_std * jax.random.normal(key=loss_parameters.key, shape=gates_logits.shape)
...
@gin.configurable
def aux_loss(loss_parameters: types.MoELossParameters, loss_fns: Any = ()) -> Iterable[types.MoELossReturn]:
