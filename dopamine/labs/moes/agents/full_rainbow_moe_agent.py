...
def train(
    ...
    def per_sample_loss_fn(params, target):
        ...
        if distributional:
            ...
            td_loss = jax.vmap(losses.softmax_cross_entropy_loss_with_logits)(target, chosen_action_logits)
        else:
    ...
    def loss_fn(params, target, loss_multipliers):
        ...
        if isinstance(net_outputs, arch_types.MoENetworkReturn):
            ...
            aux_loss_values = jnp.sum(jnp.array([aux_loss.value for aux_loss in aux_losses]))
            aux_vars.update(
                {
    ...
    if log_moe_statistics and networks.MoEType[network_def.moe_type] != networks.MoEType.BASELINE:
        moe_statistics = {}
        ...
        moe_statistics["MoE_Stats/mean_abs_weights_router"] = mean_abs_weights_router
        top_experts_one_hot = jax.nn.one_hot(aux_vars["top_experts"], network_def.num_experts)
        avg_top_experts_one_hot = jnp.mean(jnp.sum(top_experts_one_hot, axis=[1, 2]), axis=0)
        for i in range(network_def.num_experts):
            moe_statistics[f"MoE_Stats/expert{i}gate"] = experts_prob[i]
            moe_statistics[f"MoE_Stats/selected_expert{i}"] = avg_top_experts_one_hot[i]
            if networks.MoEType[network_def.moe_type] == networks.MoEType.DROPLESSMOE:
                ...
                mean_abs_grad = jnp.mean(jnp.abs(grad["params"]["SoftMoE_0"]["phi_weights"][:, i]))
            else:
                mean_abs_grad = jnp.mean(jnp.abs(grad["params"]["MoE_0"]["router"]["Dense_0"]["kernel"][i]))
    ...
    if log_network_statistics:
        ...
        batch_product_fn = lambda x: x.reshape(batch_size, -1) @ x.reshape(batch_size, -1).T
        ...
        network_statistics["param_norm"] = jnp.sum(jax.flatten_util.ravel_pytree(online_params)[0] ** 2)
        ...
        network_statistics["entk_off_diagonal_mean"] = jnp.sum(entk_off_diagonal) / (batch_size * (batch_size - 1))
        for t in [0.0, 0.001, 0.01]:
            network_statistics[f"srank_{t}"] = jnp.sum(singular_vals / jnp.max(singular_vals) > t)
            network_statistics[f"dormant_{t}"] = jnp.sum(feature_vars <= t)
            network_statistics[f"entk_srank_{t}"] = jnp.sum(entk_spectrum / jnp.max(entk_spectrum) > t)
        ...
        q_values = jax.vmap(q_online, in_axes=(0, None))(states, jax.random.PRNGKey(0)).q_values
...
class JaxFullRainbowMoEAgent(full_rainbow_agent.JaxFullRainbowAgent):
    ...
    def __init__(
        ...
        super().__init__(num_actions=num_actions, seed=seed, summary_writer=summary_writer)
    ...
    def _training_step_update(self):
        ...
        should_log_network_statistics = (
            self.log_network_statistics and (self.training_steps % self.summary_writing_frequency) == 0
        )
        train_returns = train(
            self.network_def,
            self.online_params,
        ...
        if self._replay_scheme == "prioritized":
            ...
            self._replay.set_priority(self.replay_elements["indices"], jnp.sqrt(td_loss + 1e-10))
        ...
        if (
            ...
            if hasattr(self, "collector_dispatcher"):
                statistics = [
                    statistics_instance.StatisticsInstance("CombinedLoss", np.asarray(loss), step=self.training_steps),
                ...
                self.collector_dispatcher.write(statistics, collector_allowlist=self._collector_allowlist)
