...
def train(
    ...
    if (
        ...
        mean_abs_grad_router = jnp.mean(jnp.abs(grad["params"]["MoE_0"]["router"]["Dense_0"]["kernel"]))
        mean_abs_weights_router = jnp.mean(jnp.abs(online_params["params"]["MoE_0"]["router"]["Dense_0"]["kernel"]))
...
class Atari100kRainbowMoEAgent(base_100k_rainbow.Atari100kRainbowAgent):
    ...
    def _training_step_update(self):
        ...
        next_states = self.train_preprocess_fn(self.replay_elements["next_state"], rng=rng2)
