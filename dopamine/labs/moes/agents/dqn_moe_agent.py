...
def train(
    ...
    target = target_q(q_target, next_states, rewards, terminals, cumulative_gamma, rng2)
    ...
    if (
        ...
        and (networks.MoEType[network_def.moe_type] != networks.MoEType.SIMPLICIAL_EMBEDDING_V1)
        and (networks.MoEType[network_def.moe_type] != networks.MoEType.SIMPLICIAL_EMBEDDING_V2)
    ):
        moe_statistics = {}
    ...
    if log_dormant_statistics and (networks.MoEType[network_def.moe_type] != networks.MoEType.DROPLESSMOE):
        ...
        dormancy_statistics["Stats/dormant_percentage"] = (
            (jnp.count_nonzero(score <= dormancy_threshold)).astype(float) / jnp.size(score)
        ) * 100.0
...
def target_q(target_network, next_states, rewards, terminals, cumulative_gamma, rng):
...
class DQNMoEAgent(dqn_agent.JaxDQNAgent):
    ...
    def __init__(
        ...
        logging.info("\t num_updates_per_train_step: %s", self._num_updates_per_train_step)
        ...
        if (
            ...
            if hasattr(self, "collector_dispatcher"):
                statistics = [
                    ...
                    statistics_instance.StatisticsInstance("TDLoss", np.asarray(td_loss), step=self.training_steps),
                ]
