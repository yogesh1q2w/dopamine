...
class ExpertModel(nn.Module):
    ...
    def setup(self):
        ...
        if self.noisy:
            self.net = base_networks.NoisyNetwork(rng_key=self.rng_key, eval_mode=self.eval_mode)
        else:
            self.net = nn.Dense(
...
def _maybe_create_moe_module(
    ...
    if moe_type == MoEType.SOFTMOE:
        if expert_type == "BIG":
            ...
            return softmoe.SoftMoE(
                module=ExpertModel(expert_hidden_size=num_features, noisy=noisy, rng_key=rng_key),
                num_experts=num_experts,
                expert_type=expert_type,
            )
...
class ImpalaMoE(nn.Module):
    ...
    def __call__(self, x: jax.Array, *, key: jax.Array) -> types.NetworkReturn:
        ...
        if moe_net is None:
            ...
            scale_hidden = self.scale_hidden_layer or self.moe_type == "SIMPLICIAL_EMBEDDING_V2"
            hidden_features = self.expert_hidden_units * self.num_experts if scale_hidden else self.expert_hidden_units
        ...
        return types.MoENetworkReturn(q_values=q_values, moe_out=moe_out, hidden_act=x_hidden)
...
class RainbowCNNet(nn.Module):
    ...
    def __call__(self, x):
        ...
        stride_sizes = [4, 2, 1]
        logging.info(
            ("Creating CNN network with %s kernel_sizes, %s hidden_sizes, %s" " stride_sizes, nn_scale %s"),
            str(kernel_sizes),
