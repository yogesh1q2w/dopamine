...
class SoftMoE(nn.Module):
    ...
    def __call__(self, x: jax.Array, *, key: jax.Array) -> types.MoEModuleReturn:
        ...
        if self.expert_type == "BIG":
            num_slots = int(math.ceil(num_tokens * self.capacity_factor / self.num_experts))
        ...
        if self.use_random_phi:
            ...
            phi_weights = jax.random.normal(key, (token_length, self.num_experts, num_slots))
        else:
        ...
        if self.normalization:
            ...
            phi_weights = scale_value[jnp.newaxis, jnp.newaxis, :].repeat(phi_weights.shape[0], axis=0).repeat(
                phi_weights.shape[1], axis=1
            ).repeat(phi_weights.shape[2], axis=2) * l2_normalize(phi_weights, axis=0)
        else:
            x_normalized = x
        ...
        if self.expert_type == "BIG":
            ...
            mixture_inputs = mixture_inputs.reshape(self.num_experts, 1, dim, dim, token_length)
        ...
        router_out = types.RouterReturn(
            ...
            top_experts=jnp.argmax(combine_weights.mean(axis=-1), axis=1, keepdims=True),
        )
