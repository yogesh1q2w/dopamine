...
@functools.partial(jax.vmap, in_axes=(None, None, 0, 0, 0, 0, None, None, None, None))
def target_output(
    model,
    target_network,
    next_states,
...
def train(
    ...
    def loss_fn(params, target):
        ...
        if network_def.transform is not None:
            ...
            mean_td_loss = jnp.mean(classification_loss_fn(target_probs, chosen_action_logits))
        else:
        ...
        bc_loss = jnp.mean(jax.scipy.special.logsumexp(q_values, axis=-1) - replay_chosen_q)
...
class OfflineClassyCQLAgent(full_rainbow_agent.JaxFullRainbowAgent):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            network=functools.partial(networks.ParameterizedRainbowNetwork, transform=self._transform),
        ...
        if self.training_steps > 0 and self.training_steps % self.summary_writing_frequency == 0:
            if self.summary_writer is not None:
                with self.summary_writer.as_default():
                    tf.summary.scalar("Losses/Aggregate", mean_loss, step=self.training_steps)
                    ...
                    tf.summary.scalar("Losses/DR3Loss", dr3_loss, step=self.training_steps)
                    tf.summary.scalar("Losses/MCLoss", mc_loss, step=self.training_steps)
                    tf.summary.scalar("Losses/AvgQ", avg_q_value, step=self.training_steps)
    ...
    def _build_replay_buffer(self):
        ...
        dataset_name = tfds_replay.get_atari_ds_name_from_replay(self.replay_data_dir)
        return tfds_replay.JaxFixedReplayBufferTFDS(
            replay_capacity=gin.query_parameter("JaxFixedReplayBuffer.replay_capacity"),
    ...
    def log_gradient_steps_per_epoch(self):
        ...
        if self.summary_writer is not None:
            with self.summary_writer.as_default():
                tf.summary.scalar("Info/EpochFractionSteps", steps_per_epoch, step=self.training_steps)
