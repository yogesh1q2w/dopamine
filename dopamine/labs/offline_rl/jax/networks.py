...
ClassyNetworkType = collections.namedtuple("classy_network", ["q_values", "logits", "probabilities", "representation"])
...
class CNNEncoder(nn.Module):
    ...
    def __call__(self, x):
        initializer = nn.initializers.xavier_uniform()
        for channel, ks_size, stride_size in zip(self.conv_channels, self.conv_kernels, self.conv_strides):
            x = nn.Conv(
...
class ImpalaNetworkWithRepresentations(nn.Module):
    ...
    def __call__(self, x, stop_grad_representation=False):
        ...
        # layers and units.
        logging.info(
            ("Creating Impala network with %d nn_scale, %s stack_sizes, %d " "num_blocks, normalize_representation %s"),
            self.nn_scale,
        ...
        q_values = nn.Dense(features=self.num_actions, kernel_init=initializer)(representation)
...
class ParameterizedRainbowNetwork(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, support, key=None, return_to_condition=None, eval_mode=False):
        del key, eval_mode
        if not self.inputs_preprocessed:
            x = preprocess_atari_inputs(x)
        ...
        x = nn.Dense(features=self.feature_dim * self.nn_scale, kernel_init=initializer)(x)
