...
def train(
    ...
    def loss_fn(params, bellman_target):
        ...
        next_state_representations = jnp.squeeze(next_states_model_output.representation)
        ...
        bellman_loss = jnp.mean(jax.vmap(losses.huber_loss)(bellman_target, replay_chosen_q))

        # CQL Loss
        cql_loss = jnp.mean(jax.scipy.special.logsumexp(q_values, axis=-1) - replay_chosen_q)
        loss = bellman_loss + dr3_coefficient * dr3_loss + cql_coefficient * cql_loss
    ...
    bellman_target = base_dqn_agent.target_q(q_target, next_states, rewards, terminals, cumulative_gamma)
    grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
    (loss, component_losses), grad = grad_fn(online_params, bellman_target)

    # Compute actual Bellman targets
    actual_bellman_target = base_dqn_agent.target_q(q_online_temp, next_states, rewards, terminals, cumulative_gamma)
    _, (actual_bellman_loss, _, _, _, _) = loss_fn(online_params, actual_bellman_target)
...
def compute_dr3_loss(state_representations, next_state_representations):
    ...
    dot_products = jnp.einsum("ij,ij->i", state_representations, next_state_representations)
            ...
            if self.training_steps > 0 and self.training_steps % self.summary_writing_frequency == 0:
                if self.summary_writer is not None:
                    with self.summary_writer.as_default():
                        ...
                        tf.summary.scalar("Losses/CQLLoss", cql_loss, step=self.training_steps)
                        tf.summary.scalar(
                            "Losses/BellmanActual",
                            actual_bellman_loss,
                            step=self.training_steps,
                        )
                        tf.summary.scalar("Losses/q_vals_data", q_data, step=self.training_steps)
                        tf.summary.scalar("Losses/q_vals_pi", q_pi, step=self.training_steps)
                    self.summary_writer.flush()
                if hasattr(self, "collector_dispatcher"):
                    self.collector_dispatcher.write(
                        [
                            statistics_instance.StatisticsInstance("Loss", np.asarray(loss), step=self.training_steps),
                        ],
