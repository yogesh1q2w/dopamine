...
def _parallel_lookup(game, episodes_per_policy, max_checkpoints=50, num_datasets=5):
        ...
        for i in range(1, num_datasets + 1):
            ...
            for j, chkpt in enumerate(ds):
                ...
                if int(chkpt.split("_")[1]) < max_checkpoints:
                    job = executor.submit(_get_lookups, i, j, ds[chkpt], episodes_per_policy)
                    jobs.append(job)
...
def choose_indices(
    ...
    sorted_policy_ids = list(sorted(policy_ids, key=lambda pid: policy_value_lookup[pid]))
    inexpert_policy_ids = sorted_policy_ids[: int(dataset_expertise * num_ids)]
    if returns_filter is not None:
        inexpert_policy_ids = inexpert_policy_ids[int(returns_filter * len(inexpert_policy_ids)) :]

    if len(inexpert_policy_ids) < dataset_policies:
        selected_policy_ids = inexpert_policy_ids
    else:
        selected_policy_ids = [
            inexpert_policy_ids[int(i)] for i in np.linspace(0, len(inexpert_policy_ids) - 1, dataset_policies)
        ]
    ...
    for policy_id in selected_policy_ids:
        ...
        all_episode_idxs.append((policy_id[0], policy_id[1], f"{start_idx}:{end_idx}"))
        # Book keeping
