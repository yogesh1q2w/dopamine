...
def get_transition_dataset_fn(stack_size, update_horizon=1, gamma=0.99, return_to_go=False):
...
def create_atari_ds_loader(
    ...
    shuffled_dataset = dataset.shuffle(shuffle_num_steps, reshuffle_each_iteration=True)
...
def build_tfds_replay(
    ...
    transition_fn = get_transition_dataset_fn(stack_size, update_horizon, gamma, return_to_go=return_to_go)
    if dataset is None:
