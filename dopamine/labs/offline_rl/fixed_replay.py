...
class JaxFixedReplayBuffer(fixed_replay_buffer.FixedReplayBuffer):
    ...
    def __init__(
        ...
        logging.info("Creating %s with the following parameters:", self.__class__.__name__)
        logging.info("\t replay_capacity: %d", replay_capacity)
        logging.info("\t replay_transitions_start_index %d", replay_transitions_start_index)
    ...
    def _load_buffer(self, suffix):
        ...
        try:
            ...
            logging.info("Starting to load from ckpt %d from %s", int(suffix), self._data_dir)
            ...
            end_index = self._replay_transitions_start_index + replay_capacity + replay_buffer._stack_size
            ...
            for name, array in replay_buffer._store.items():
                ...
                replay_buffer._store[name] = array[self._replay_transitions_start_index : end_index].copy()
