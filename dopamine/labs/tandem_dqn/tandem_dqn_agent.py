...
def target_q(
    ...
    if double_dqn:
        next_state_q_vals_for_argmax = jax.vmap(online_network, in_axes=(0))(next_states).q_values
    else:
        next_state_q_vals_for_argmax = jax.vmap(target_network, in_axes=(0))(next_states).q_values
...
class TandemDQNAgent(dqn_agent.JaxDQNAgent):
    ...
    def _build_networks_and_optimizer(self):
        ...
        self.active_optimizer_state = self.active_optimizer.init(self.active_online_params)
        ...
        self.passive_optimizer_state = self.passive_optimizer.init(self.passive_online_params)
    ...
    def _train_step(self):
        ...
        if self._replay.add_count > self.min_replay_history:
            if self.training_steps % self.update_period == 0:
                ...
                self.active_optimizer_state, self.active_online_params, active_loss = dqn_agent.train(
                    self.network_def,
                ...
                if (
                    ...
                    with self.summary_writer.as_default():
                        tf.summary.scalar("Losses/Active", active_loss, step=self.training_steps)
                        tf.summary.scalar("Losses/Passive", passive_loss, step=self.training_steps)
