...
@gin.configurable
def create_tandem_agents_and_checkpoints(sess, environment, agent_name="dqn", summary_writer=None, debug_mode=False):
    ...
    del debug_mode
    if agent_name == "dqn":
        return tandem_dqn_agent.TandemDQNAgent(num_actions=environment.action_space.n, summary_writer=summary_writer)
    else:
...
class TandemRunner(run_experiment.Runner):
    ...
    def _run_one_phase(self, min_steps, statistics, run_mode_str, agent_type="active"):
        ...
        while step_count < min_steps:
            ...
            statistics.append(
                {
                    "{}_{}_episode_lengths".format(run_mode_str, agent_type): episode_length,
                    "{}_{}_episode_returns".format(run_mode_str, agent_type): episode_return,
                }
            )
    ...
    def _run_eval_phase(self, statistics, agent_type="active"):
        ...
        _, sum_returns, num_episodes = self._run_one_phase(self._evaluation_steps, statistics, "eval", agent_type)
    ...
    def _run_one_iteration(self, iteration):
        ...
        active_num_episodes_eval, active_average_reward_eval = self._run_eval_phase(statistics, "active")
        passive_num_episodes_eval, passive_average_reward_eval = self._run_eval_phase(statistics, "passive")
