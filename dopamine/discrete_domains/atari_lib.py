...
ImplicitQuantileNetworkType = collections.namedtuple("iqn_network", ["quantile_values", "quantiles"])
...
def create_atari_environment(
    ...
    continuous_action_threshold = 0.0 if continuous_action_threshold is None else continuous_action_threshold
    env = gym.make(
        full_game_name,
...
#         quantiles = tf.random.uniform(quantiles_shape, minval=0, maxval=1, dtype=tf.float32)
