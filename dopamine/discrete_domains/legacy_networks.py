...
def implicit_quantile_network(num_actions, quantile_embedding_dim, network_type, state, num_quantiles):
...
@gin.configurable
def _basic_discrete_domain_network(min_vals, max_vals, num_actions, state, num_atoms=None):
    ...
    """
    layer = gym_lib.BasicDiscreteDomainNetwork(min_vals, max_vals, num_actions, num_atoms)
    return layer(state)
...
def fourier_dqn_network(min_vals, max_vals, num_actions, state, fourier_basis_order=3):
    ...
    Returns:
      The Q-values for DQN-style agents or logits for Rainbow-style agents.
    """
    model = gym_lib.FourierDQNNetwork(min_vals, max_vals, num_actions, fourier_basis_order)
