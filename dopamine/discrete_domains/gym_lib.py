...
def create_gym_environment(
    ...
    if use_legacy_gym:
        ...
        if use_ppo_preprocessing:
            ...
            env = legacy_gym.wrappers.TransformObservation(env, lambda obs: np.clip(obs, -10, 10))
            env = legacy_gym.wrappers.NormalizeReward(env)
            env = legacy_gym.wrappers.TransformReward(env, lambda reward: np.clip(reward, -10, 10))
    else:
...
class BasicDiscreteDomainNetwork(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self.dense1 = tf.keras.layers.Dense(512, activation=activation_fn, name="fully_connected")
        self.dense2 = tf.keras.layers.Dense(512, activation=activation_fn, name="fully_connected")
        if num_atoms is None:
            self.last_layer = tf.keras.layers.Dense(num_actions, name="fully_connected")
        else:
            self.last_layer = tf.keras.layers.Dense(num_actions * num_atoms, name="fully_connected")
...
class FourierBasis(object):
    ...
    def __init__(self, nvars, min_vals=0, max_vals=None, order=3):
        ...
        self.multipliers = tf.constant([list(map(int, x)) for x in terms][1:], dtype=tf.float32)
...
class FourierDQNNetwork(tf.keras.Model):
    ...
    def __init__(self, min_vals, max_vals, num_actions, fourier_basis_order=3, name=None):
        ...
        # Defining layers.
        self.flatten = tf.keras.layers.Flatten()
        self.last_layer = tf.keras.layers.Dense(num_actions, use_bias=False, name="fully_connected")
...
class CartpoleFourierDQNNetwork(FourierDQNNetwork):
    ...
    def __init__(self, num_actions, name=None):
        ...
        Args:
          ...
          name: str, used to create scope for network parameters.
        """
        super(CartpoleFourierDQNNetwork, self).__init__(CARTPOLE_MIN_VALS, CARTPOLE_MAX_VALS, num_actions, name=name)
...
class CartpoleRainbowNetwork(tf.keras.Model):
    ...
    def __init__(self, num_actions, num_atoms, support, name=None):
        ...
        self.net = BasicDiscreteDomainNetwork(CARTPOLE_MIN_VALS, CARTPOLE_MAX_VALS, num_actions, num_atoms=num_atoms)
        self.num_actions = num_actions
        self.num_atoms = num_atoms
...
class AcrobotDQNNetwork(tf.keras.Model):
    ...
    def __init__(self, num_actions, name=None):
        ...
        self.net = BasicDiscreteDomainNetwork(ACROBOT_MIN_VALS, ACROBOT_MAX_VALS, num_actions)
...
class AcrobotFourierDQNNetwork(FourierDQNNetwork):
    ...
    def __init__(self, num_actions, name=None):
        ...
        super(AcrobotFourierDQNNetwork, self).__init__(ACROBOT_MIN_VALS, ACROBOT_MAX_VALS, num_actions, name=name)
...
class AcrobotRainbowNetwork(tf.keras.Model):
    ...
    def __init__(self, num_actions, num_atoms, support, name=None):
        ...
        self.net = BasicDiscreteDomainNetwork(ACROBOT_MIN_VALS, ACROBOT_MAX_VALS, num_actions, num_atoms=num_atoms)
        self.num_actions = num_actions
        self.num_atoms = num_atoms
        self.support = support
...
class MountainCarDQNNetwork(tf.keras.Model):
    ...
    def __init__(self, num_actions, name=None):
        ...
        self.net = BasicDiscreteDomainNetwork(MOUNTAINCAR_MIN_VALS, MOUNTAINCAR_MAX_VALS, num_actions)
...
class GymPreprocessing(object):
    ...
    def step(self, action):
        if self._use_legacy_gym:
            ...
            observation, reward, game_over, truncated, info = self.environment.step(action)
