...
@gin.configurable
def get_latest_checkpoint_number(base_directory, override_number=None, sentinel_file_identifier="checkpoint"):
...
class Checkpointer(object):
    ...
    def __init__(
        ...
        self._sentinel_file_prefix = "sentinel_{}_complete".format(sentinel_file_identifier)
        ...
        try:
            ...
            raise ValueError("Unable to create checkpoint path: {}.".format(base_directory)) from permission_error
    ...
    def _clean_up_old_checkpoints(self, iteration_number):
        ...
        # versions old.
        stale_iteration_number = iteration_number - (self._checkpoint_frequency * self._checkpoint_duration)

        # If keep_every has been set, we spare every keep_every'th checkpoint
        if self._keep_every is not None and (
            stale_iteration_number % (self._keep_every * self._checkpoint_frequency) == 0
        ):
            return

        if stale_iteration_number >= 0:
            stale_file = self._generate_filename(self._checkpoint_file_prefix, stale_iteration_number)
            stale_sentinel = self._generate_filename(self._sentinel_file_prefix, stale_iteration_number)
            try:
    ...
    def load_checkpoint(self, iteration_number):
        ...
        """
        checkpoint_file = self._generate_filename(self._checkpoint_file_prefix, iteration_number)
