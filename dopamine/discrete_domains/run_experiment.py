...
def create_agent(sess, environment, agent_name=None, summary_writer=None, debug_mode=False):
    ...
    if agent_name.startswith("dqn"):
        ...
        return jax_dqn_agent.JaxDQNAgent(num_actions=environment.action_space.n, summary_writer=summary_writer)
        ...
        return jax_rainbow_agent.JaxRainbowAgent(num_actions=environment.action_space.n, summary_writer=summary_writer)
...
class Runner(object):
    ...
    def __init__(
        ...
        fine_grained_print_to_console=False,
    ):
        ...
        self._agent = create_agent_fn(self._sess, self._environment, summary_writer=self._base_dir)
        ...
        self._collector_dispatcher = collector_dispatcher.CollectorDispatcher(self._base_dir)
        set_collector_dispatcher_fn = getattr(self._agent, "set_collector_dispatcher", None)
    ...
    def _create_directories(self):
        ...
        if self._use_legacy_logger:
            logging.warning("DEPRECATION WARNING: Logger is being deprecated. " "Please switch to CollectorDispatcher!")
    ...
    def _run_one_phase(self, min_steps, statistics, run_mode_str):
        ...
        while step_count < min_steps:
            if self._max_steps_per_episode is None:
                episode_length, episode_return = self._run_continued_episode(step_count, min_steps)
            else:
    ...
    def _run_one_iteration(self, iteration):
        ...
        if self._has_collector_dispatcher:
            self._collector_dispatcher.write(
                [
                    statistics_instance.StatisticsInstance("Train/NumEpisodes", num_episodes_train, iteration),
                    statistics_instance.StatisticsInstance("Train/AverageReturns", average_reward_train, iteration),
                    statistics_instance.StatisticsInstance(
                        "Train/AverageStepsPerSecond", average_steps_per_second, iteration
                    ),
                    statistics_instance.StatisticsInstance("Eval/NumEpisodes", num_episodes_eval, iteration),
                    statistics_instance.StatisticsInstance("Eval/AverageReturns", average_reward_eval, iteration),
                ]
            )
...
class TrainRunner(Runner):
    ...
    def _save_tensorboard_summaries(self, iteration, num_episodes, average_reward, average_steps_per_second):
        """Save statistics as tensorboard summaries."""
        if self._summary_writer is None:
            return
        ...
        if self._sess is None:
            with self._summary_writer.as_default():
                ...
                tf.summary.scalar("Train/AverageReturns", average_reward, step=iteration)
                tf.summary.scalar(
                    "Train/AverageStepsPerSecond",
                    average_steps_per_second,
                    step=iteration,
            ...
            summary = tf.compat.v1.Summary(
                value=[
                    ...
                    tf.compat.v1.Summary.Value(tag="Train/AverageReturns", simple_value=average_reward),
                    tf.compat.v1.Summary.Value(
                        tag="Train/AverageStepsPerSecond",
                        simple_value=average_steps_per_second,
                    ),
