...
class ActorNetworkTest(parameterized.TestCase):
    ...
    def test_actor_network_outputs_correct_shaped_values(self):
        ...
        mode, sampled_action, action_probability = network_def.apply(params, self.example_state, self.key)
    ...
    def test_network_has_specified_number_of_layers(self):
        num_layers = 5
        network_def = continuous_networks.ActorNetwork(self.action_shape, num_layers)
        params = network_def.init(self.key, self.example_state, self.key)
    ...
    @parameterized.named_parameters(("relu", nn.relu), ("sigmoid", nn.sigmoid), ("tanh", nn.tanh))
    def test_network_activation_initializer(self, activation):
        network = continuous_networks.ActorNetwork(self.action_shape, activation=activation)
    ...
    def test_network_kernel_initializer(self, kernel_initializer):
        network = continuous_networks.ActorNetwork(self.action_shape, kernel_initializer=kernel_initializer)
...
class CriticNetworkTest(parameterized.TestCase):
    ...
    def test_network_kernel_initializer(self, kernel_initializer):
        network = continuous_networks.CriticNetwork(kernel_initializer=kernel_initializer)
        self.assertEqual(network.kernel_initializer, kernel_initializer)
...
class ActorCriticNetworkTest(absltest.TestCase):
    ...
    def setUp(self):
        ...
        self.network_def = continuous_networks.ActorCriticNetwork(self.action_shape, self.num_layers)
        self.params = self.network_def.init(self.key, self.example_state, self.key)

    def assert_actor_shapes_are_correct(self, actor_output: continuous_networks.ActorOutput):
    ...
    def assert_critic_shapes_are_correct(self, critic_output: continuous_networks.CriticOutput):
    ...
    def test_actor_critic_network_actor_outputs_correct_shaped_values(self):
        output = self.network_def.apply(self.params, self.example_state, self.key, method=self.network_def.actor)
    ...
    def test_network_has_specified_number_of_layers(self):
        actor_layer_names = {x for x in self.params["params"]["_actor"] if x.startswith("Dense_")}
        with self.subTest("actor"):
            # n hidden layers + 1 output layer.
            self.assertLen(actor_layer_names, self.num_layers + 1)

        critic1_layer_names = {x for x in self.params["params"]["_critic1"] if x.startswith("Dense_")}
        critic2_layer_names = {x for x in self.params["params"]["_critic2"] if x.startswith("Dense_")}
...
class PPOActorNetworkTest(parameterized.TestCase):
    ...
    def test_ppo_actor_network_outputs_correct_shaped_values(self):
        ...
        sampled_action, action_probability, entropy = network_def.apply(params, self.example_state, self.key)
    ...
    def test_ppo_network_has_specified_number_of_layers(self):
        num_layers = 5
        network_def = continuous_networks.PPOActorNetwork(self.action_shape, num_layers)
        params = network_def.init(self.key, self.example_state, self.key)
    ...
    def test_ppo_network_activation_initializer(self, activation):
        network = continuous_networks.PPOActorNetwork(self.action_shape, activation=activation)
        self.assertEqual(network.activation, activation)

    def test_scale_diag_set_to_zero(self):
        num_layers = 4
        hidden_units = 256
        network = continuous_networks.PPOActorNetwork(self.action_shape, num_layers, hidden_units)
...
class PPOActorCriticNetworkTest(absltest.TestCase):
    ...
    def setUp(self):
        ...
        self.network_def = continuous_networks.PPOActorCriticNetwork(self.action_shape, self.num_layers)
        self.params = self.network_def.init(self.key, self.example_state, self.key)

    def assert_ppo_actor_shapes_are_correct(self, actor_output: continuous_networks.PPOActorOutput):
        with self.subTest(name="sampled_action"):
            self.assertEqual(actor_output.sampled_action.shape, self.action_shape)
        with self.subTest(name="log_probability"):
            self.assertEqual(actor_output.log_probability.shape, ())
    ...
    def assert_ppo_critic_shapes_are_correct(self, critic_output: continuous_networks.PPOCriticOutput):
    ...
    def test_network_has_specified_number_of_layers(self):
        ...
        critic_layer_names = {x for x in self.params["params"]["_critic"] if x.startswith("Dense_")}
