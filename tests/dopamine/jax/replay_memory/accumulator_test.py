...
class TransitionAccumulatorTest(parameterized.TestCase):
    ...
    def _verify_accumulator_transition(
        ...
        # the stack.
        np.testing.assert_array_equal(accumulator_transition.observation, transition.observation)
        ...
        self.assertEqual(accumulator_transition.is_terminal, int(transition.is_terminal))
        self.assertEqual(accumulator_transition.episode_end, int(transition.episode_end))
    ...
    def testInitializer(self):
        acc = accumulator.TransitionAccumulator(self._stack_size, self._update_horizon, self._gamma)
    ...
    def testReset(self):
        ...
        for i in range(self._update_horizon + self._stack_size + 1):
            self._add(acc, elements.TransitionElement(self._obs * i, i, i, False, False))
    ...
    def testAddSingleTrajectory(self, n: int, valid: bool):
        ...
        if not valid:
            ...
            self.assertEqual(
                timesteps[0].reward,
                np.sum(self._sample_transition.reward * (self._gamma ** np.arange(n))),
            )
    ...
    def testAccumulate(self, n):
        ...
        # We add enough transitions for a valid trajectory.
        for i in range(8):
            element = list(acc.accumulate(elements.TransitionElement(self._obs * i, i, i, False, False)))
            if i > n - 1:
    ...
    def testAccumulateWithInvalidFirstTrajectory(self):
        ...
        # return None.
        element = list(acc.accumulate(elements.TransitionElement(self._obs, 1, 1, True, False)))
        self.assertEmpty(element)
        ...
        # trajectory.
        element = list(acc.accumulate(elements.TransitionElement(self._obs * 2, 2, 2, False, False)))
        self.assertEmpty(element)
        ...
        element = list(acc.accumulate(elements.TransitionElement(self._obs * 3, 3, 3, False, False)))
        self.assertLen(element, 1)
        self.assertIsNotNone(element[0])
