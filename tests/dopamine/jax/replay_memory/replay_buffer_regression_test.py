...
class ReplayBufferRegressionTest(parameterized.TestCase):
    ...
    def testNSteprewards(self):
        ...
        for i in range(50):
            transition = elements.TransitionElement(np.full(OBSERVATION_SHAPE, i, dtype=OBS_DTYPE), 0, 2.0, False)
    ...
    def testGetStack(self):
        ...
        for i in range(11):
            transition = elements.TransitionElement(np.full(OBSERVATION_SHAPE, i, dtype=OBS_DTYPE), 0, 0, False)
            memory.add(transition)

        # ensure that the returned shapes are always correct
        for i in memory._memory:
            np.testing.assert_array_equal(memory._memory[i].state.shape, OBSERVATION_SHAPE + (4,))
    ...
    def testSampleTransitionBatch(self):
        ...
        for i in range(num_adds):
            ...
            transition = elements.TransitionElement(np.full(OBSERVATION_SHAPE, i, OBS_DTYPE), 0, 0, terminal, False)
            memory.add(transition)
            if not terminal:
        ...
        indices = rng.integers(len(sampler._key_by_index), size=len(sampler._key_by_index))

        def make_state(key: int) -> np.ndarray:
            return np.full(OBSERVATION_SHAPE + (1,), key, dtype=OBS_DTYPE)

        expected_states = np.array([make_state(index_to_id[sampler._key_by_index[i]]) for i in indices])
        expected_next_states = np.array([make_state(index_to_id[sampler._key_by_index[i]] + 1) for i in indices])
        ...
        expected_terminal = np.array([int(((index_to_id[sampler._key_by_index[i]] + 1) % 4) == 0) for i in indices])
