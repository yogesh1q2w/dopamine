...
class ReplayBufferTest(parameterized.TestCase):
    ...
    def setUp(self):
        ...
        self._sample_transition = elements.TransitionElement(self._obs, 1, 3.14, False, False)
    ...
    def testAddWithValidAccumulatorReturn(self, mock_accumulator):
        ...
        np.testing.assert_array_equal(replay._memory[0].observation, self._sample_transition.observation)
        ...
        self.assertEqual(replay._memory[0].is_terminal, int(self._sample_transition.is_terminal))
        self.assertEqual(replay._memory[0].episode_end, int(self._sample_transition.episode_end))
    ...
    def testAddUpToCapacity(self, mock_accumulator):
        ...
        transitions = []
        for i in range(15):
            transitions.append(elements.TransitionElement(self._obs * i, i, i, False, False))
        ...
        for i in expected_keys:
            np.testing.assert_array_equal(replay._memory[i].observation, transitions[i].observation)
            ...
            self.assertEqual(replay._memory[i].is_terminal, int(transitions[i].is_terminal))
            self.assertEqual(replay._memory[i].episode_end, int(transitions[i].episode_end))
    ...
    @parameterized.named_parameters(
        ...
        dict(testcase_name="DefaultBSCompress", use_default_bs=True, compress=True),
        dict(
    ...
    def testSample(self, mock_accumulator, use_default_bs, compress):
        ...
        for i, idx in enumerate(indices):
            np.testing.assert_array_equal(batches.state[i], transitions[idx].observation)
            np.testing.assert_array_equal(batches.next_state[i], np.ones_like(self._obs) * (idx + 1))
    ...
    def testKeyMappingsForSampling(self):
        ...
        self.assertEqual(next_key, sampler._key_by_index[sampler._index_by_key[next_key]])
