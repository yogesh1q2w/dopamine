...
class DQNAgentTest(absltest.TestCase):
    ...
    def testPreprocessFnParam(self):
        ...
        agent = MockDQNAgent(num_actions=5, network=MockNetwork, preprocess_fn=preprocess_fn)
    ...
    def testBeginEpisode(self):
        ...
        self.assertTrue(np.array_equal(agent._observation, first_observation[:, :, 0]))
        ...
        self.assertTrue(np.array_equal(agent.state, expected_state))
        self.assertTrue(np.array_equal(agent._observation, second_observation[:, :, 0]))
    ...
    def testStepTrain(self):
        ...
        for step in range(1, num_steps + 1):
            ...
            self.assertTrue(np.array_equal(last_observation[:, :, 0], mock_args[0].observation))
        ...
        mock_args, _ = agent._replay.add.call_args
        self.assertTrue(np.array_equal(observation[:, :, 0], mock_args[0].observation))
        self.assertEqual(0, mock_args[0].action)  # Action selected.
        self.assertEqual(1, mock_args[0].reward)  # Reward received.
    ...
    def testLinearlyDecayingEpsilon(self):
        ...
        for step, expected_epsilon in steps_schedule:
            np.testing.assert_almost_equal(
                dqn_agent.linearly_decaying_epsilon(decay_period, step, warmup_steps, epsilon),
                expected_epsilon,
                0.01,
            )
