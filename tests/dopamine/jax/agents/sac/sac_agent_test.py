...
def get_mock_batch(
    ...
    return elements.ReplayElement(
        ...
        next_state=np.stack([mock_observation for _ in range(batch_size)], axis=0),
        is_terminal=np.stack([mock_terminal for _ in range(batch_size)], axis=0),
        episode_end=np.stack([mock_episode_end for _ in range(batch_size)], axis=0),
    ), samplers.PrioritizedSampleMetadata(
        keys=np.stack([mock_indices for _ in range(batch_size)], axis=0),
        probabilities=np.stack([mock_sampling_prob for _ in range(batch_size)], axis=0),
    )
...
class SacAgentTest(parameterized.TestCase):
    ...
    def assertAgentParametersEqual(self, agent1: sac_agent.SACAgent, agent2: sac_agent.SACAgent):
        ...
        self.assertEqual(agent1_structure, agent2_structure, "Parameter structures do not match.")
    ...
    def assertAgentParametersNotEqual(self, agent1: sac_agent.SACAgent, agent2: sac_agent.SACAgent):
        agent1_params = get_agent_params(agent1)
        agent2_params = get_agent_params(agent2)
    ...
    def testTupleShapedActionsMatchShapes(self, action_shape: Tuple[int, ...], eval_mode: bool):
        agent = create_agent(action_shape=action_shape, eval_mode=eval_mode)
        observation = np.zeros(OBSERVATION_SHAPE)
    ...
    def testAgentParametersUpdateWhenTrained(self, action_shape: Tuple[int, ...]):
        ...
        agent._replay.sample = mock.MagicMock(return_value=get_mock_batch(action_shape))
