...
class PPOAgentTest(parameterized.TestCase):
    ...
    def assertAgentParametersEqual(self, agent1: ppo_agent.PPOAgent, agent2: ppo_agent.PPOAgent):
    ...
    def assertAgentParametersNotEqual(self, agent1: ppo_agent.PPOAgent, agent2: ppo_agent.PPOAgent):
        agent1_params = get_agent_params(agent1)
        agent2_params = get_agent_params(agent2)
    ...
    def test_tuple_shaped_actions_match_shapes(self, action_shape: Tuple[int, ...], eval_mode: bool):
        agent = create_agent(action_shape=action_shape, eval_mode=eval_mode)
        observation = np.zeros(OBSERVATION_SHAPE)
    ...
    def test_calculate_advantages_and_returns_discounting(self, discounts, gamma, lambda_):
        batch_shape = (5,)
        q_values = jnp.zeros(batch_shape)
        next_q_value = jnp.array(0.0)
    ...
    def test_create_minibatches_and_shuffle_shapes(self):
        ...
        self.assertEqual(states.shape, (num_batches, batch_size) + OBSERVATION_SHAPE)
    ...
    def test_select_action_shapes(self, action_shape):
        ...
        rng, action = ppo_agent.select_action(agent.network_def, agent.network_params, state, rng)
