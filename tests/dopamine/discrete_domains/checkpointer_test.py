...
class CheckpointerTest(tf.test.TestCase):
    ...
    def testCheckpointingInitialization(self):
        ...
        with self.assertRaisesRegex(ValueError, "No path provided to Checkpointer."):
            checkpointer.Checkpointer("")
        # Fails with invalid directory.
        invalid_dir = "/does/not/exist"
        with self.assertRaisesRegex(ValueError, "Unable to create checkpoint path: {}.".format(invalid_dir)):
    ...
    def testLogToFileWithValidDirectoryCustomPrefix(self):
        ...
        tmpdir = self.create_tempdir()
        exp_checkpointer = checkpointer.Checkpointer(tmpdir, checkpoint_file_prefix=prefix)
        data = {"data1": 1, "data2": "two", "data3": (3, "three")}
        iteration_number = 1729
        exp_checkpointer.save_checkpoint(iteration_number, data)
        loaded_data = exp_checkpointer.load_checkpoint(iteration_number)
    ...
    def testLoadLatestCheckpointWithInvalidDir(self):
        self.assertEqual(-1, checkpointer.get_latest_checkpoint_number("/does/not/exist"))
    ...
    def testLoadLatestCheckpointWithOverride(self):
        ...
        self.assertEqual(
            override_number,
            checkpointer.get_latest_checkpoint_number("/ignored", override_number=override_number),
        )
    ...
    def testLoadLatestCheckpoint(self):
        ...
        self.assertEqual(second_iter, checkpointer.get_latest_checkpoint_number(tmpdir))

    def testGarbageCollection(self):
        custom_prefix = "custom_prefix"
        tmpdir = self.create_tempdir()
        exp_checkpointer = checkpointer.Checkpointer(tmpdir, checkpoint_file_prefix=custom_prefix)
        data = {"data1": 1, "data2": "two", "data3": (3, "three")}
        deleted_log_files = 7
    ...
    def testGarbageCollectionWithCheckpointFrequency(self):
        ...
        data = {"data1": 1, "data2": "two", "data3": (3, "three")}
        deleted_log_files = 6
        total_log_files = (exp_checkpointer._checkpoint_duration * checkpoint_frequency) + deleted_log_files + 1
