...
class RunExperimentTest(tf.test.TestCase):
    ...
    def testCreateDQNAgent(self, mock_dqn_agent):
        ...
        self.assertEqual(
            70,
            run_experiment.create_agent(self.test_session(), environment, agent_name="dqn"),
        )
    ...
    def testCreateRainbowAgent(self, mock_rainbow_agent):
        ...
        self.assertEqual(
            70,
            run_experiment.create_agent(self.test_session(), environment, agent_name="rainbow"),
        )
    ...
    def testCreateImplicitQuantileAgent(self, mock_implicit_quantile_agent):
        ...
        self.assertEqual(
            70,
            run_experiment.create_agent(self.test_session(), environment, agent_name="implicit_quantile"),
        )
...
class RunnerTest(tf.test.TestCase):
    ...
    def setUp(self):
        ...
        def create_agent_fn(unused_x, unused_y, summary_writer):
            ...
            self._agent.summary_writer = tf.compat.v1.summary.FileWriter(summary_writer)
    ...
    def testInitializeCheckpointingWhenCheckpointUnbundleFails(
        ...
        runner = run_experiment.Runner(self._test_subdir, lambda x, y, summary_writer: agent, mock.Mock)
    ...
    @mock.patch.object(checkpointer, "get_latest_checkpoint_number")
    def testInitializeCheckpointingWhenCheckpointUnbundleSucceeds(self, mock_get_latest):
        latest_checkpoint = 7
        mock_get_latest.return_value = latest_checkpoint
        logs_data = {"a": 1, "b": 2}
        ...
        runner = run_experiment.Runner(self._test_subdir, lambda x, y, summary_writer: mock_agent, mock.Mock)
        ...
        mock_agent.unbundle.assert_called_once_with(checkpoint_dir, latest_checkpoint, checkpoint_data)
    ...
    def testRunOneContinuedEpisode(self):
        ...
        step_number, total_reward = runner._run_continued_episode(start_step_count=0, max_step_count=10)
    ...
    def testRunOneContinuedEpisodeWithLowMaxSteps(self):
        ...
        step_number, total_reward = runner._run_continued_episode(start_step_count=0, max_step_count=1)
    ...
    def testRunOnePhase(self):
        ...
        statistics = []
        runner = run_experiment.Runner(self._test_subdir, self._create_agent_fn, lambda: environment)
        step_number, sum_returns, num_episodes = runner._run_one_phase(max_steps, statistics, "test")
        ...
        for _ in range(calls_to_run_episode):
            expected_statistics.append({"test_episode_lengths": 2, "test_episode_returns": -1})
    ...
    def testLogExperiment(self, mock_logger_constructor):
        ...
        self.assertEqual((num_iterations / log_every_n), experiment_logger._calls_to_log)
    ...
    @mock.patch.object(checkpointer, "Checkpointer")
    @mock.patch.object(logger, "Logger")
    def testCheckpointExperiment(self, mock_logger_constructor, mock_checkpointer_constructor):
        checkpoint_dir = os.path.join(self._test_subdir, "checkpoints")
        test_dict = {"test": 1}
        iteration = 1729
        ...
        mock_logger_constructor.return_value = mock_logger
        runner = run_experiment.Runner(self._test_subdir, self._create_agent_fn, mock.Mock)
    ...
    @mock.patch.object(checkpointer, "Checkpointer")
    @mock.patch.object(logger, "Logger")
    def testRunExperimentWithInconsistentRange(self, mock_logger_constructor, mock_checkpointer_constructor):
        experiment_logger = MockLogger()
        mock_logger_constructor.return_value = experiment_logger
        experiment_checkpointer = mock.Mock()
        mock_checkpointer_constructor.return_value = experiment_checkpointer
        runner = run_experiment.Runner(self._test_subdir, self._create_agent_fn, mock.Mock, num_iterations=0)
    ...
    def testRunExperiment(
        ...
        runner.run_experiment()
        self.assertEqual(num_iterations, experiment_checkpointer.save_checkpoint.call_count)
        self.assertEqual(num_iterations, experiment_logger._calls_to_set)
    ...
    def testCollectorDispatcherSetup(self, mock_collector_dispatcher):
        ...
        _ = run_experiment.Runner(self._test_subdir, self._create_agent_fn, lambda: environment)
        ...
        self.assertEqual("CD", self._agent.set_collector_dispatcher.call_args_list[0][0][0])
