...
class MockDeepmindControlSuiteEnvironment(control.Environment):
    ...
    def observation_spec(self) -> Mapping[str, specs.BoundedArray]:
        ...
        result["obs1"] = specs.BoundedArray((2,), np.float32, np.asarray([-1.0, -1.0]), np.asarray([1.0, 1.0]))
...
class DeepmindControlLibTest(parameterized.TestCase):
    ...
    def test_preprocessing(self):
        ...
        np.testing.assert_equal(env.action_space.low, np.full((4,), -1.0, dtype=np.float32))
        np.testing.assert_equal(env.action_space.high, np.full((4,), 1.0, dtype=np.float32))
    ...
    def test_float_image_observation_is_scaled_correctly_to_uint(self, fill_value: float, expected: int):
        env = MockDeepmindControlSuiteEnvironment(
    ...
    def test_action_repeats_successfully_apply_repeated_action(self):
        ...
        gym_env = deepmind_control_lib.DeepmindControlPreprocessing(control_env, action_repeat=action_repeat)
