...
class NetworksTest(parameterized.TestCase):
    ...
    def testExpertModel(self, maintain_token_size):
        ...
        expected_output_size = (self._vector_state.shape[0] if maintain_token_size else self._expert_hidden_size,)
    ...
    def testImpalaMoEWithInvalidMoEType(self):
        net = networks.ImpalaMoE(num_actions=4, inputs_preprocessed=True, moe_type="FOOBAR")
        with self.assertRaises(AssertionError):
            _ = net.init(self._rng, x=self._pixel_state, key=self._rng)

    def testImpalaMoEWithInvalidRoutingType(self):
        net = networks.ImpalaMoE(num_actions=4, inputs_preprocessed=True, routing_type="FOOBAR")
        with self.assertRaises(AssertionError):
            _ = net.init(self._rng, x=self._pixel_state, key=self._rng)
    ...
    def _create_network_and_apply(self, network_class, moe_type, state, support=None):
        net = network_class(
            num_actions=self._num_actions,
            inputs_preprocessed=True,
    ...
    def _create_impala_network_and_apply(self, moe_type, routing_type):
        return self._create_network_and_apply(
            network_class=functools.partial(networks.ImpalaMoE, routing_type=routing_type, patch_size=(4, 4)),
            moe_type=moe_type,
    ...
    def _test_network_outputs(
        ...
        np.testing.assert_equal(net_out.moe_out.values.shape, expected_moe_output_shape)
        np.testing.assert_equal(router_out.output.shape, expected_router_out_shape)
        np.testing.assert_equal(router_out.probabilities.shape, expected_router_out_shape)
        np.testing.assert_equal(router_out.top_expert_weights.shape, expected_expert_weights_shape)
        np.testing.assert_equal(router_out.top_experts.shape, expected_top_expert_shape)
    ...
    def testNatureDQNMoEWithInvalidMoEType(self):
        net = networks.NatureDQNMoE(num_actions=4, inputs_preprocessed=True, moe_type="FOOBAR")
        with self.assertRaises(AssertionError):
            _ = net.init(self._rng, x=self._pixel_state, key=self._rng)

    def testNatureDQNMoEWithInvalidRoutingType(self):
        net = networks.NatureDQNMoE(num_actions=4, inputs_preprocessed=True, routing_type="FOOBAR")
        with self.assertRaises(AssertionError):
            _ = net.init(self._rng, x=self._pixel_state, key=self._rng)
    ...
    def _create_nature_network_and_apply(self, moe_type, routing_type):
        return self._create_network_and_apply(
            network_class=functools.partial(networks.NatureDQNMoE, routing_type=routing_type, patch_size=(4, 4)),
            moe_type=moe_type,
            state=self._pixel_state,
        )
    ...
    def _create_full_rainbow_network_and_apply(self, moe_type, routing_type):
        return self._create_network_and_apply(
            network_class=functools.partial(
                networks.FullRainbowMoENetwork,
    ...
    def testFullRainbowSoftMoE(self, routing_type, expected_moe_output_shape):
        net_out = self._create_full_rainbow_network_and_apply("SOFTMOE", routing_type)
        self._test_network_outputs(
            net_out,
            expected_moe_output_shape,
            expected_router_out_shape=(expected_moe_output_shape[0], 5),
