...
class OfflineAgentTest(parameterized.TestCase):
    ...
    def setUp(self):
        ...
        gin.bind_parameter("fixed_replay.JaxFixedReplayBuffer.batch_size", batch_size)
    ...
    def _create_agent_fn(self, agent_name):
        if agent_name == "dr3":
            ...
            gin.bind_parameter("JaxDQNAgent.network", networks.JAXDQNNetworkWithRepresentations)
    ...
    def _test_train_step_updates_weights(self, agent_name):
        ...
        agent._sample_from_replay_buffer = mock.create_autospec(agent._sample_from_replay_buffer, spec_set=True)
        ...
        for i, (param1, param2) in enumerate(zip(jax.tree.leaves(params_before), jax.tree.leaves(params_after))):
    ...
    @parameterized.parameters(ClassyLoss.HL_GAUSS, ClassyLoss.TWO_HOT, ClassyLoss.SCALAR)
