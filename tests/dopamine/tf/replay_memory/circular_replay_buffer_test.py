...
class OutOfGraphReplayBufferTest(tf.test.TestCase):
    ...
    def testGetRangeInvalidIndexOrder(self):
        ...
        with self.assertRaisesRegex(AssertionError, "end_index must be larger than start_index"):
    ...
    def testGetStack(self):
        ...
        for i in range(3, memory.cursor()):
            self.assertTrue(memory.get_observation_stack(i).shape, OBSERVATION_SHAPE + (4,))
        ...
        for i in range(4):
            self.assertTrue(np.array_equal(np.full(OBSERVATION_SHAPE, i), stack[:, :, i]))
    ...
    def testSampleTransitionBatch(self):
        ...
        for i in range(num_adds):
            memory.add(np.full(OBSERVATION_SHAPE, i, OBS_DTYPE), 0, 0, i % 4)  # Every 4 transitions is terminal.
        ...
        indices = [1, 2, 3, 5, 8]
        expected_states = np.array([np.full(OBSERVATION_SHAPE + (1,), i, dtype=OBS_DTYPE) for i in indices])
        ...
        expected_terminal = np.array([min((x + num_adds - replay_capacity) % 4, 1) for x in indices])
    ...
    def testSave(self):
        ...
        for attr in memory.__dict__:
            if attr.startswith("_"):
                continue
            stale_filename = os.path.join(self._test_subdir, "{}_ckpt.{}.gz".format(attr, stale_iteration))
        ...
        for attr in memory.__dict__:
            if attr.startswith("_"):
                continue
            filename = os.path.join(self._test_subdir, "{}_ckpt.{}.gz".format(attr, current_iteration))
    ...
    def testEpisodeEndIndicesAreCorrectlySaved(self):
        ...
        filename = os.path.join(testdir.full_path, f"episode_end_indices_ckpt.{current_iteration}.gz")
        self.assertTrue(tf.io.gfile.exists(filename))
    ...
    def testSaveWithKeepEvery(self):
        ...
        for iteration in range(1, total_iterations + 1):
            for attr in memory.__dict__:
                if attr.startswith("_"):
                    continue
                filename = os.path.join(self._test_subdir, "{}_ckpt.{}.gz".format(attr, iteration))
    ...
    def testLoadFromNonexistentDirectory(self):
        ...
        self.assertFalse(np.array_equal(memory._store["observation"], self._test_observation))
        ...
        self.assertFalse(np.array_equal(memory._store["terminal"], self._test_terminal))
        self.assertFalse(np.array_equal(memory.add_count, self._test_add_count))
        self.assertFalse(np.array_equal(memory.invalid_range, self._test_invalid_range))
...
class WrappedReplayBufferTest(tf.test.TestCase):
    ...
    def testConstructorCapacityNotLargeEnough(self):
        with self.assertRaisesRegex(
            ValueError,
            r"Update horizon \(5\) should be significantly " r"smaller than replay capacity \(5\)\.",
        ):
    ...
    def testConstructorWithZeroUpdateHorizon(self):
        with self.assertRaisesRegex(ValueError, r"Update horizon must be positive\."):
            circular_replay_buffer.WrappedReplayBuffer(
                observation_shape=OBSERVATION_SHAPE,
                stack_size=STACK_SIZE,
                update_horizon=0,
    ...
    def testWrapperLoad(self):
        ...
        self.assertFalse(np.array_equal(replay.memory._store["observation"], self._test_observation))
        self.assertFalse(np.array_equal(replay.memory._store["action"], self._test_action))
        self.assertFalse(np.array_equal(replay.memory._store["reward"], self._test_reward))
        self.assertFalse(np.array_equal(replay.memory._store["terminal"], self._test_terminal))
        self.assertFalse(np.array_equal(replay.memory.add_count, self._test_add_count))
        self.assertFalse(np.array_equal(replay.memory.invalid_range, self._test_invalid_range))
        store_prefix = "$store$_"
        numpy_arrays = {
            store_prefix + "observation": self._test_observation,
            store_prefix + "action": self._test_action,
