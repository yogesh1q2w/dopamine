...
class OutOfGraphPrioritizedReplayBufferTest(tf.test.TestCase):
    ...
    def testAddWithAdditionalArgsAndPriority(self):
        memory = self.create_default_memory(
            extra_storage_types=[circular_replay_buffer.ReplayElement("test_item", (), np.float32)]
        )
        self.assertEqual(memory.cursor(), 0)
        zeros = np.zeros(SCREEN_SIZE)
    ...
    def testGetPriorityWithInvalidIndices(self):
        ...
        with self.assertRaises(AssertionError, msg="Indices must be int32s, given: int64"):
    ...
    def testLowPriorityElementNotFrequentlySampled(self):
        ...
        for _ in range(100):
            _, _, _, _, _, _, terminals, _, _ = memory.sample_transition_batch(batch_size=2)
    ...
    def testSampleIndexBatchTooManyFailedRetries(self):
        ...
        with self.assertRaises(
            RuntimeError,
            msg=("Max sample attempts: Tried 10 times but only sampled 1 valid " "indices. Batch size is 2"),
        ):
