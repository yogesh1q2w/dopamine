...
class SumTreeTest(tf.test.TestCase):
    ...
    def testNegativeCapacity(self):
        with self.assertRaises(ValueError, msg="Sum tree capacity should be positive. Got: -1"):
            sum_tree.SumTree(capacity=-1)

    def testSetNegativeValue(self):
        with self.assertRaises(ValueError, msg="Sum tree values should be nonnegative. Got -1"):
