...
class ProjectDistributionTest(tf.test.TestCase):
    ...
    def testInconsistentSupportsAndWeightsParameters(self):
        ...
        weights = tf.constant([[0.1, 0.2, 0.3, 0.2], [0.1, 0.2, 0.3, 0.2]], dtype=tf.float32)
    ...
    def testInconsistentSupportsAndWeightsWithPlaceholders(self):
        ...
        projection = rainbow_agent.project_distribution(supports_ph, weights_ph, target_support_ph, validate_args=True)
    ...
    def testInconsistentSupportsAndTargetSupportParameters(self):
        supports = tf.constant([[0, 2, 4, 6, 8], [3, 4, 5, 6, 7]], dtype=tf.float32)
        weights = tf.constant([[0.1, 0.2, 0.3, 0.2, 0.2], [0.1, 0.2, 0.3, 0.2, 0.2]], dtype=tf.float32)
    ...
    def testProjectWithNonMonotonicTargetSupport(self):
        ...
        projection = rainbow_agent.project_distribution(supports, weights, target_support, validate_args=True)
        with self.test_session() as sess:
            tf.compat.v1.global_variables_initializer().run()
            with self.assertRaisesRegex(tf.errors.InvalidArgumentError, "assertion failed"):
                sess.run(projection)
    ...
    def testProjectSingleIdenticalDistribution(self):
        ...
        projection = rainbow_agent.project_distribution(supports, weights, target_support)
        with self.test_session() as sess:
            tf.compat.v1.global_variables_initializer().run()
    ...
    def testExampleFromCodeComments(self):
        ...
        weights = tf.constant([[0.1, 0.6, 0.1, 0.1, 0.1], [0.1, 0.2, 0.5, 0.1, 0.1]], dtype=tf.float32)
        target_support = tf.constant([4, 5, 6, 7, 8], dtype=tf.float32)
        projection = rainbow_agent.project_distribution(supports, weights, target_support)
    ...
    def testProjectBatchOfDifferentDistributions(self):
        supports = tf.constant([[0, 2, 4, 6, 8], [0, 1, 2, 3, 4], [3, 4, 5, 6, 7]], dtype=tf.float32)
        weights = tf.constant(
            [
                [0.1, 0.2, 0.3, 0.2, 0.2],
                [0.1, 0.2, 0.1, 0.3, 0.3],
    ...
    def testUsingPlaceholders(self):
        ...
        target_support_ph = tf.compat.v1.placeholder(tf.float32, None)
        projection = rainbow_agent.project_distribution(supports_ph, weights_ph, target_support_ph)
        expected_projections = [
            [0.3, 0.3, 0.0, 0.2, 0.2],
            [0.7, 0.3, 0.0, 0.0, 0.0],
            [0.1, 0.2, 0.3, 0.2, 0.2],
    ...
    def testProjectBatchOfDifferentDistributionsWithLargerDelta(self):
        ...
        weights = tf.constant([[0.1, 0.2, 0.2, 0.2, 0.3], [0.1, 0.2, 0.4, 0.1, 0.2]], dtype=tf.float32)
...
class RainbowAgentTest(tf.test.TestCase):
    ...
    def _create_test_agent(self, sess):
        ...
        class MockRainbowNetwork(tf.keras.Model):
            ...
            def __init__(self, num_actions, num_atoms, support, **kwargs):
                ...
                bottom_rows = np.tile(np.ones(self.num_actions * self.num_atoms), (stack_size - 1, 1))
            ...
            def call(self, state):
                inputs = tf.constant(np.zeros((state.shape[0], stack_size)), dtype=tf.float32)
    ...
    def testShapesAndValues(self):
        with tf.compat.v1.Session() as sess:
            ...
            self.assertEqual(agent._replay_net_outputs.logits.shape[1], self._num_actions)
            ...
            self.assertEqual(agent._replay_next_target_net_outputs.logits.shape[2], self._num_atoms)
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for step in range(1, num_steps + 1):
                ...
                if stack_pos >= -self.stack_size:
                    expected_state[:, :, :, stack_pos] = np.full((1,) + self.observation_shape, step)
            self.assertAllEqual(agent.state, expected_state)
            self.assertAllEqual(
    ...
    def testStoreTransitionWithUniformSampling(self):
        with tf.compat.v1.Session() as sess:
            agent = rainbow_agent.RainbowAgent(sess, num_actions=4, replay_scheme="uniform")
    ...
    def testStoreTransitionWithPrioritizedSamplingy(self):
        with tf.compat.v1.Session() as sess:
            agent = rainbow_agent.RainbowAgent(sess, num_actions=4, replay_scheme="prioritized")
            dummy_frame = np.zeros((84, 84))
            # Adding transitions with default, 10., default priorities.
            agent._store_transition(dummy_frame, 0, 0, False)
            agent._store_transition(dummy_frame, 0, 0, False, 10.0)
