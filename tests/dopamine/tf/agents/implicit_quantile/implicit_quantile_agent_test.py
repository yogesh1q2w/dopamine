...
class ImplicitQuantileAgentTest(tf.test.TestCase):
    ...
    def _create_test_agent(self, sess):
        ...
        class MockImplicitQuantileNetwork(tf.keras.Model):
            ...
            def call(self, state, num_quantiles):
                ...
                return atari_lib.ImplicitQuantileNetworkType(self.layer(inputs), quantiles)
    ...
    def testShapes(self):
        with self.test_session(use_gpu=False) as sess:
            ...
            self.assertEqual(agent._net_outputs.quantile_values.shape[1], agent.num_actions)
            ...
            self.assertEqual(agent._replay_net_quantile_values.shape[1], agent.num_actions)

            self.assertEqual(
                agent._replay_net_target_quantile_values.shape[0],
                agent.num_tau_prime_samples * agent._replay.batch_size,
            )
            self.assertEqual(agent._replay_net_target_quantile_values.shape[1], agent.num_actions)

            self.assertEqual(agent._replay_net_target_q_values.shape[0], agent._replay.batch_size)
            self.assertEqual(agent._replay_net_target_q_values.shape[1], agent.num_actions)
    ...
    def test_replay_quantile_value_computation(self):
        with self.test_session(use_gpu=False) as sess:
            ...
            batch_size = agent._replay.batch_size
            replay_quantile_vals = replay_quantile_vals.reshape([agent.num_tau_samples, batch_size, agent.num_actions])
            ...
            for i in range(agent.num_tau_prime_samples):
                for j in range(agent._replay.batch_size):
                    self.assertEqual(replay_target_quantile_vals[i][j][0], agent.num_actions)
